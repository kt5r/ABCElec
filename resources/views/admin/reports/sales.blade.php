...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900">{{ __('Daily Sales Report') }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('Total Orders') }}</h3>
            <p class="text-3xl font-bold text-indigo-600">{{ $dailySales->total_orders ?? 0 }}</p>
        </div>

        <!-- Total Sales -->
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('Total Sales') }}</h3>
            <p class="text-3xl font-bold text-indigo-600">LKR {{ number_format($dailySales->total_sales ?? 0, 2) }}</p>
        </div>

        <!-- Average Order Value -->
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('Average Order Value') }}</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <!-- Sales by Product -->
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ __('Sales by Product') }}</h2>
        ...
        <!-- Sales by Hour -->
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ __('Sales by Hour') }}</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
        ...
        <!-- Payment Methods -->
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ __('Payment Methods') }}</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
